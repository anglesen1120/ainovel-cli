...
func TestAggregateABDeltaControlsGate(t *testing.T) {
	...
	if cr.Outcome != Warn {
		t.Fatalf("delta warning phải làm case WARN, nhận %s", cr.Outcome)
	}
	if cr.Summary.VariantPassRate != 1 || cr.Summary.DeltaPassRate != 0 {
		t.Fatalf("AB summary không đúng: %+v", cr.Summary)
	}
	s := Aggregate("run", "ab", "writer-x", 1, []CaseResult{cr})
	if s.Gate != Warn {
		t.Fatalf("suite gate = %s nhưng cần WARN", s.Gate)
	}
}
