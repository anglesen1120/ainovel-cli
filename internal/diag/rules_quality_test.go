...
func TestPayoffMissPattern(t *testing.T) {
	snap := &Snapshot{
		Plans: map[int]*domain.ChapterPlan{
			1: {Chapter: 1, Contract: domain.ChapterContract{PayoffPoints: []string{""}}},
			2: {Chapter: 2, Contract: domain.ChapterContract{PayoffPoints: []string{""}}},
			3: {Chapter: 3, Contract: domain.ChapterContract{PayoffPoints: []string{""}}},
		},
		Reviews: map[int]*domain.ReviewEntry{
