...
func TestCallStructuredCarriesRawOnSemanticFailure(t *testing.T) {
	m := &nativeImportModel{mockModel: &mockModel{responses: []string{"đầu ra lỗi not json"}}}
	...
	if se.Raw != "đầu ra lỗi not json" || !strings.Contains(se.Error(), "Phân tích JSON thất bại") {
