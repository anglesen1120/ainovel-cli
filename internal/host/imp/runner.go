...
func (r *runner) checkSourceIdentity() error {
	...
	if Digest(raw) != m.RawSHA256 {
		return fmt.Errorf("đã có một lần nhập %q đang chạy, nhưng file nguồn lần này có nội dung khác: hãy hoàn tất hoặc bỏ dở lần nhập cũ trước (xóa meta/import/) rồi nhập sách mới", m.SourceName)
	}
	return nil
}
...
func (r *runner) confirm() bool {
	...
	if !auto && !accept {
		...
		if blockedByNotes {
			msg += "  ! Có ghi chú dung sai phân đoạn, --yes không tự động cho qua, vui lòng kiểm tra thủ công\n"
		}
