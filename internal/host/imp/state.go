...
func LoadState(w *Workspace) (Facts, error) {
	...
	if err != nil {
		return f, fmt.Errorf("đọc artifact chia đoạn: %w", err)
	}
	...
	if err != nil {
		return f, fmt.Errorf("đọc nguyên văn artifact chia đoạn: %w", err)
	}
	confirmed, err := artifactFresh[Confirmation](w, fileConfirmation, Digest(segRaw))
	if err != nil {
		return f, fmt.Errorf("đọc xác nhận chia đoạn: %w", err)
	}
	if !confirmed {
		return f, nil
	}
