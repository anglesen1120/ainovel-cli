...
//
// Cấu trúc gói (OEBPS là container OPS):
//
...
// zipDeflate ghi một entry thông thường (có nén).
...
// splitParagraphs tách đoạn theo dòng trống; nhiều dòng trống liên tiếp được xem là một điểm tách.
// Mỗi đoạn trả về đã TrimSpace và không rỗng. Xuống dòng trong đoạn (một \n) được giữ thành dấu cách vì <p> XHTML không giữ newline; trình đọc tự wrap.
func splitParagraphs(body string) []string {
	...
	for _, p := range parts {
		...
		// Đổi xuống dòng trong đoạn thành dấu cách để tránh mất nội dung khi render XHTML.
...
`)
	...
	// Dùng danh sách chương phẳng. Gom nhóm quyển/hồi trong trình đọc thường kém thoáng hơn mục lục một tầng
	// (trình đọc cũng tự thu gọn), và EPUB 3 nav với ol lồng nhau render lạ trên vài trình đọc. Giữ đơn giản.
	for _, ch := range chapters {
		title := strings.TrimSpace(titleIdx[ch])
...
// bookIdentifier sinh chuỗi UUID ổn định từ tên tiểu thuyết.
//
// **Chỉ dùng novelName, không trộn danh sách chương**: danh tính tác phẩm nên gắn với "đây là sách nào",
// không gắn với "phạm vi xuất" hay "tại thời điểm xuất đã viết tới chương mấy". Xuất lại cùng một sách
// giữ nguyên ID để trình đọc nhận ra đây là bản cập nhật của cùng tác phẩm (trạng thái cập nhật do timestamp
// dcterms:modified đảm nhiệm). novelName rỗng dùng chung ID là edge case đã biết: nếu người dùng không đặt tên
// cho hai sách khác nhau thì tự chịu trách nhiệm.
func bookIdentifier(novelName string) string {
	h := sha1.New()
	h.Write([]byte(novelName))
	sum := h.Sum(nil)
	// Định dạng kiểu UUID (8-4-4-4-12), không cần đúng RFC 4122 nghiêm ngặt; EPUB chỉ cần chuỗi duy nhất và ổn định.
