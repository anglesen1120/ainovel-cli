...
func coCreateStream(ctx context.Context, models *bootstrap.ModelSet, sessions *store.SessionStore, sysPrompt string, history []CoCreateMessage, onProgress func(kind, text string)) (reply CoCreateReply, err error) {
	if len(history) == 0 {
		return CoCreateReply{}, fmt.Errorf("lịch sử cocreate trống")
	}
	...
	// Để điều tra các vấn đề ngẫu nhiên như "cocreate empty response", cần thấy mô hình thực sự trả về gì.
	...
	if err != nil {
		return CoCreateReply{}, fmt.Errorf("tạo nội dung cocreate: %w", err)
	}
	...
	for ev := range streamCh {
		...
		case agentcore.StreamEventError:
			if ev.Err != nil {
				return CoCreateReply{}, fmt.Errorf("tạo nội dung cocreate: %w", ev.Err)
			}
			return CoCreateReply{}, fmt.Errorf("tạo nội dung cocreate thất bại")
		}
	}

	// Channel fallback: các mô hình thiên về suy luận (R1/GLM-Z1/QwQ v.v.) đôi khi ghi toàn bộ câu trả lời vào
	// reasoning_content rồi không chuyển lại kênh final answer, khiến raw rỗng nhưng thinking chứa
	// đầy đủ bốn đoạn. Trường hợp thực tế xem meta/sessions/cocreate.jsonl -- lấy thẳng thinking làm raw để phân tích,
	// tầng giao thức đã có xử lý hạ cấp (khi không có marker [REPLY] thì coi cả đoạn là reply), sau khi cứu tình huống, trải nghiệm UI không khác biệt.
	rawText := raw.String()
...
func parseCoCreateResponse(raw string) (CoCreateReply, error) {
	...
	if raw == "" {
		return CoCreateReply{}, fmt.Errorf("phản hồi cocreate trống")
	}
