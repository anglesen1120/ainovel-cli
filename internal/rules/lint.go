...
// Lint kiểm tra các giới hạn sản phẩm tích hợp sẵn: quét phần văn bản để tìm dấu vết cơ chế, không liên quan đến quy tắc người dùng; luôn thực hiện khi commit.
// Cùng hợp đồng với Check — chỉ trả về dữ kiện (nguyên tắc sắt một), không chặn luồng; để người đánh giá/người dùng quyết định.
//
// Hiện có ba loại (đều là các khiếm khuyết đã được kiểm chứng từ sản phẩm chạy đường dài thực tế):
