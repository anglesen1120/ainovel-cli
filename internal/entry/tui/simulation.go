...
func (s *simulationState) refresh(contentW int) {
	...
	b.WriteString(titleStyle.Render("Log quy trình"))
	b.WriteString(" ")
	...
	case s.err != nil:
		b.WriteString(errStyle.Render("Xử lý hồ sơ mô phỏng viết thất bại"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc đóng panel"))
	default:
		b.WriteString(okStyle.Render("Hồ sơ mô phỏng viết đã sẵn sàng, Agent sau sẽ đọc từ novel_context"))
		b.WriteString("\n")
		b.WriteString(dimStyle.Render("Esc đóng panel"))
	}
...
func renderSimulationModal(width, height int, s *simulationState) string {
	...
	modal := renderPaddedModalFrame(boxW, boxH, "Hồ sơ mô phỏng viết", hint, strings.Split(s.viewport.View(), "\n"))
...
func startSimulate(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Tạo Hồ sơ mô phỏng viết", "./simulate", width, height, cancel)
...
func startImportSimulation(rt *host.Host, reqID int, args []string, width, height int) (*simulationState, tea.Cmd, error) {
	...
	state := newSimulationState(reqID, "Nhập Hồ sơ mô phỏng viết", args[0], width, height, cancel)
	return state, listenSimulationEvent(reqID, ch), nil
}
