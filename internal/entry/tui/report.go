...
type reportState struct {
	reqID      int
	report     *diag.Report
	exportPath string // đường dẫn file chẩn đoán đã khử nhạy cảm, hiển thị ở đầu báo cáo để dán vào issue
	exportErr  error
	loading    bool
	renderW    int
	startedAt  time.Time
...
func renderReportText(report diag.Report, width int, exportPath string, exportErr error, startedAt, finishedAt time.Time) string {
	...
	// chẩn đoán đã khử nhạy cảm đã được xuất → hướng dẫn người dùng dán vào issue
	if exportPath != "" {
		...
		b.WriteString(lipgloss.NewStyle().Foreground(colorError).Render("Xuất chẩn đoán đã khử nhạy cảm thất bại：" + exportErr.Error()))
		b.WriteString("\n\n")
	}
	...
	// Dòng thứ hai: Rà soát + Viết lại + Điểm TB
	b.WriteString(mutedStyle.Render("Rà soát "))
	...
	// Dòng thứ ba: manh mối cài cắm + Quy hoạch
	if st.ForeshadowOpen > 0 || st.ForeshadowStale > 0 {
		b.WriteString(mutedStyle.Render("manh mối cài cắm "))
...
func renderReportLoadingText(width int, startedAt time.Time) string {
	...
	b.WriteString("\n\n")
	b.WriteString(bodyStyle.Render(wrapText("Đang đọc sản phẩm output của tiểu thuyết hiện tại và phân tích các vấn đề về quy trình, chất lượng, Quy hoạch và ngữ cảnh. Khi dự án lớn có thể cần vài giây.", width)))
	b.WriteString("\n\n")
	b.WriteString(hintStyle.Render("Esc có thể Tắt bảng trước, sau khi phân tích nền hoàn tất sẽ tạo lại vào lần mở tiếp theo."))
	return b.String()
}
...
// wrapText thực hiện xuống dòng đơn giản cho văn bản dài.
func wrapText(s string, maxWidth int) string {
	...
	lineW := 0
	for _, r := range s {
		// Tại chỗ xuống dòng vốn có phải đặt lại độ rộng dòng: độ rộng của '\n' là 0, nếu không đặt lại sẽ khiến độ rộng tích lũy
		// của thông báo nhiều dòng bị đánh giá nhầm là quá rộng, từ dòng đầu tiên bị xuống dòng trở đi sẽ chèn xuống dòng giả + thụt lề vào mỗi dòng phía sau (làm vỡ toàn bộ).
		if r == '\n' {
			b.WriteRune(r)
			lineW = 0
			continue
...
func renderReportModal(width, height int, state *reportState) string {
	...
	// Nếu kích thước viewport thay đổi, cập nhật
