...
type modelSwitchState struct {
	...
	// initialThinkingKey ghi lại giá trị Trung bình ban đầu của trường cường độ Vai trò này khi bảng điều khiển mở.
	// Chỉ khi người dùng thực sự thay đổi trường này mới ghi ngược trở lại——ý định cường độ đã lưu có thể Cao hơn năng lực mô hình hiện tại, bảng điều khiển không thể hiển thị, không thể vì “không động” mà xóa nhầm.
...
func (s *modelSwitchState) apply(rt modelRuntime) error {
	...
	if len(s.models) == 0 {
		return fmt.Errorf("provider %q không có mô hình đã cấu hình", s.provider())
	}
	wantThinking := s.thinkingKey()
	if err := rt.SwitchModel(s.role(), s.provider(), s.model()); err != nil {
		return err
	}
	// Mức suy luận và mô hình là trực giao: chỉ khi người dùng thực sự thay đổi trường cường độ thì mới ghi ngược trở lại, tránh biến ý định Cao mà bảng điều khiển không thể hiển thị
	// do năng lực mô hình hiện tại không đủ thành giá trị mặc định ban đầu một cách nhầm lẫn.
