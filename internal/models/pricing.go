...
const (
	...
	// maxModelAgeDays giữ nhất quán với gen_models.go: model quá tuổi này được xem là lỗi thời và bị loại.
	maxModelAgeDays = 730
)

// providerMap chuẩn hóa tiền tố vendor của OpenRouter thành tên provider cục bộ.
// Vendor không liệt kê sẽ bị bỏ qua để tránh kéo về mục không dùng được.
...
// StartPricingRefresh khởi chạy goroutine nền để làm mới dữ liệu model.
// Đọc cache đĩa trước với TTL 24h; nếu hết hạn hoặc không có thì kéo dữ liệu mới và ghi xuống đĩa.
// Khi cacheDir rỗng, bỏ qua cache đĩa nhưng vẫn thử kéo qua mạng.
...
// isStaleModel lọc model lỗi thời theo maxModelAgeDays.
// Giá trị 0 hoặc âm được xem là thiếu dữ liệu và bị loại như model cũ.
...
// tokenToMillion đổi giá USD/token do OpenRouter trả về thành USD/1M token.
