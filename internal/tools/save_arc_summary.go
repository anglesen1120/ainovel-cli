...
func (t *SaveArcSummaryTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if !replay {
		...
		// Tóm tắt cung là dấu hoàn tất của Router, được ghi như công件 ngữ nghĩa cuối cùng. Trước đó bất kỳ bước nào
...
// arcSummaryReplay chỉ cho qua phần kết thúc idempotent có nội dung hoàn toàn giống nhau, dùng khi công件 ngữ nghĩa đã được ghi xuống nhưng
// checkpoint thêm thất bại cần được thử lại. Mọi khác biệt đều là xung đột rõ ràng, không thể dùng thử lại để ghi đè các факты tổng hợp lịch sử.
func (t *SaveArcSummaryTool) arcSummaryReplay(
	...
	if !reflect.DeepEqual(*existing, summary) ||
		...
		return false, fmt.Errorf("Tóm tắt cung %d của tập %d đã tồn tại nhưng công件 liên quan khác nhau, từ chối ghi đè: %w", summary.Volume, summary.Arc, errs.ErrToolConflict)
	}
	return true, nil
}
