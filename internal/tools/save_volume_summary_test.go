...
func TestSaveVolumeSummaryRejectsNonDueVolume(t *testing.T) {
	...
	if _, err := NewSaveVolumeSummaryTool(s).Execute(context.Background(), args); err == nil || !strings.Contains(err.Error(), "mục tiêu ghi aggregate không khớp") {
		t.Fatalf("mong đợi từ chối tập chưa đến lượt, nhận được %v", err)
	}
...
func TestReconcileLayeredCompletionRepairsInterruptedVolumeSummary(t *testing.T) {
	s := setupVolumeSummaryStore(t)
	// Mô phỏng tiến trình thoát ra khi tóm tắt tập đã được ghi xuống nhưng Progress.MarkComplete chưa chạy.
