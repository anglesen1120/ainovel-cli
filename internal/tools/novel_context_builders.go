...
func (t *ContextTool) buildArchitectFoundation(envelope *architectContextEnvelope, reads *contextReads) {
	...
	// Kho phản hồi Writer: độ lệch/đề xuất đại cương do commit_chapter lưu lại, bắt buộc tham khảo khi lập kế hoạch cung/tập tiếp theo;
	// tự động xóa sau khi expand_arc / append_volume / update_compass thành công (đã tiêu thụ).
