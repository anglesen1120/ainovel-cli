...
func (t *SaveReviewTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if r.Chapter <= 0 {
		return nil, fmt.Errorf("chapter phải lớn hơn 0")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("tải tiến độ: %w", err)
	}
	if progress == nil || !slices.Contains(progress.CompletedChapters, r.Chapter) {
		return nil, fmt.Errorf("chương %d cần được hoàn thành trước khi đánh giá", r.Chapter)
	}
	...
	case "arc":
		...
		if err != nil {
			return nil, fmt.Errorf("tải đánh giá cung: %w", err)
		}
	...
	case "global":
		...
		if err != nil {
			return nil, fmt.Errorf("tải đánh giá toàn cục: %w", err)
		}
	}
	if existing != nil {
		if !reflect.DeepEqual(*existing, r) {
			return nil, fmt.Errorf("đánh giá tổng hợp của chương %d đã tồn tại với nội dung khác, từ chối ghi đè: %w", r.Chapter, errs.ErrToolConflict)
		}
	...
	// Trước tiên áp dụng nguyên tử trạng thái điều khiển, sau đó mới lưu hiện vật đánh giá. Nếu bước hai thất bại, ý định sửa lại vẫn còn;
	// sau khi Writer dọn hết hàng đợi, router sẽ vì thiếu hiện vật đánh giá mà phân phát lại Editor, không bỏ qua phần đánh giá.
	latest, err := t.store.Progress.ApplyReviewOutcome(reviewOutcome, affected, r.Summary)
	if err != nil {
		return nil, fmt.Errorf("áp dụng kết quả đánh giá: %w", err)
	}
	if err := t.store.World.SaveReview(r); err != nil {
		return nil, fmt.Errorf("lưu đánh giá: %w", err)
	}
...
func (t *SaveReviewTool) finishReview(
	...
	if _, err := t.store.Checkpoints.AppendArtifact(scope, "review", artifact); err != nil {
		return nil, fmt.Errorf("ghi checkpoint đánh giá: %w", err)
	}
...
func (t *SaveReviewTool) normalizeReviewEntry(r *domain.ReviewEntry) (*store.ArcBoundary, error) {
	...
	if len(r.AffectedChapters) > 0 {
		return nil, fmt.Errorf("affected_chapters được suy ra từ issues[].chapters; không được truyền trường này")
	}
	...
	for _, miss := range r.ContractMisses {
		if strings.TrimSpace(miss) == "" {
			return nil, fmt.Errorf("contract_misses không được chứa mục rỗng")
		}
	}
	...
	if r.Scope == "arc" {
		...
		if boundary == nil || !boundary.IsArcEnd || boundary.EndChapter != r.Chapter {
			return nil, fmt.Errorf("chương đánh giá cung phải là điểm kết thúc của cung")
		}
	}
	...
	for i := range r.Issues {
		...
		if strings.TrimSpace(issue.Description) == "" {
			return nil, fmt.Errorf("mô tả vấn đề là bắt buộc")
		}
		if strings.TrimSpace(issue.Evidence) == "" {
			return nil, fmt.Errorf("bằng chứng vấn đề là bắt buộc")
		}
		switch issue.Severity {
		case "critical", "error", "warning":
		default:
			return nil, fmt.Errorf("mức độ nghiêm trọng của vấn đề không hợp lệ: %q", issue.Severity)
		}
		if len(issue.Chapters) == 0 && r.Scope == "chapter" {
			issue.Chapters = []int{r.Chapter}
		}
		if len(issue.Chapters) == 0 {
			return nil, fmt.Errorf("các chương có vấn đề là bắt buộc khi scope=%s", r.Scope)
		}
		...
		for _, chapter := range issue.Chapters {
			...
			case "chapter":
				if chapter != r.Chapter {
					return nil, fmt.Errorf("vấn đề trong đánh giá chương phải tham chiếu chương %d, nhưng nhận được %d", r.Chapter, chapter)
				}
			case "global":
				if chapter <= 0 || chapter > r.Chapter {
					return nil, fmt.Errorf("chương %d của vấn đề đánh giá toàn cục nằm ngoài phạm vi 1-%d", chapter, r.Chapter)
				}
			case "arc":
				if chapter < boundary.StartChapter || chapter > boundary.EndChapter {
					return nil, fmt.Errorf("chương %d của vấn đề đánh giá cung nằm ngoài phạm vi %d-%d", chapter, boundary.StartChapter, boundary.EndChapter)
				}
			}
	...
	if r.Verdict == "accept" && len(derived) > 0 {
		return nil, fmt.Errorf("đánh giá accept không được chứa vấn đề có requires_change=true")
	}
	if (r.Verdict == "rewrite" || r.Verdict == "polish") && len(derived) == 0 {
		return nil, fmt.Errorf("verdict=%s cần ít nhất một vấn đề có requires_change=true", r.Verdict)
	}
...
func validateDimensions(dimensions []domain.DimensionScore) error {
	...
	for _, dim := range dimensions {
		...
		if dim.Score < 0 || dim.Score > 100 {
			return fmt.Errorf("điểm số không hợp lệ cho %s: %d", dim.Dimension, dim.Score)
		}
		if strings.TrimSpace(dim.Comment) == "" {
			return fmt.Errorf("nhận xét dimension là bắt buộc: %s", dim.Dimension)
		}
	}
	return nil
}
