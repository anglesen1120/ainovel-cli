...
// Dùng chung logic phán định store.FoundationMissing với công cụ save_foundation, bảo đảm LLM từ
// novel_context thấy ready/missing trong novel_context khớp với foundation_ready do save_foundation trả về
...
// buildRelatedChapters tra cứu các chương trước liên quan đến chương hiện tại từ dữ liệu có cấu trúc.
// Đề xuất theo bốn chiều: phục bút, nhân vật xuất hiện, thay đổi trạng thái và quan hệ; loại trùng, tối đa 5 mục.
...
func (t *ContextTool) buildRelatedChapters(
	...
	// Ghép văn bản dàn ý để so khớp từ khóa
	outlineText := entry.Title + " " + entry.CoreEvent
	for _, s := range entry.Scenes {
		outlineText += " " + s
	}

	// 1. Tra cứu phục bút: mô tả phục bút đang hoạt động có liên quan đến dàn ý chương hiện tại không
	for _, f := range foreshadow {
		if strings.Contains(outlineText, f.ID) || containsAny(outlineText, strings.Fields(f.Description)) {
			add(f.PlantedAt, fmt.Sprintf("phục bút %s (chương gieo %s)", f.ID, truncateRunes(f.Description, 15)))
		}
	...
	// 2. Tra cứu nhân vật xuất hiện: duyệt một lần theo lô, giảm IO từ O(số nhân vật×số chương) xuống O(số chương)
	chars, err := t.store.Characters.Load()
	if err != nil {
	...
	if len(outlineChars) > 0 {
		...
		for _, name := range outlineChars {
			...
			if ch, ok := appearances[name]; ok {
				add(ch, fmt.Sprintf("nhân vật '%s' xuất hiện lần cuối ở chương", name))
			}
		}
	}

	// 3. Tra cứu thay đổi trạng thái: thao tác trên slice đã tải, không IO
	for _, name := range outlineChars {
		if len(results) >= maxResults {
			break
		}
	...
	// 4. Tra cứu quan hệ: thay đổi gần nhất của quan hệ giữa các cặp nhân vật trong chương hiện tại
	if len(relationships) > 0 && len(outlineChars) >= 2 {
		...
		for _, r := range relationships {
			...
			if aIn && bIn {
				add(r.Chapter, fmt.Sprintf("quan hệ giữa %s và %s thay đổi", r.CharacterA, r.CharacterB))
			}
		}
	}
...
// findLastStateChange tìm số chương thay đổi gần nhất của thực thể trong danh sách thay đổi trạng thái đã tải.
...
// containsAny kiểm tra text có chứa bất kỳ từ nào trong words không (chỉ khớp từ ít nhất 2 ký tự để tránh nhiễu).
...
func (t *ContextTool) selectStoryThreads(state contextBuildState) []domain.RecallItem {
	...
	picked := make(map[string]struct{}) // ID phục bút đã chọn, dùng để điền bù theo tuổi và loại trùng
	...
	// 1. Gọi lại theo mức độ liên quan: phục bút trùng với các từ focus của chương hiện tại.
	...
	for _, entry := range state.foreshadow {
		...
		add(domain.RecallItem{
			...
			Summary: fmt.Sprintf("phục bút “%s” được gieo ở chương %d: %s", entry.ID, entry.PlantedAt, truncateRunes(entry.Description, 30)),
		})
	...
	// 2. Nhắc các phục bút đã quá lâu chưa xuất hiện trong ngữ cảnh, theo thứ tự được gieo.
	//    Đây là tín hiệu nhắc nhớ, không phải yêu cầu bắt buộc phải xử lý.
	for _, entry := range agingForeshadow(state.foreshadow, state.chapter, picked) {
		add(domain.RecallItem{
			Kind:    "story_thread",
			Key:     entry.ID,
			Chapter: entry.PlantedAt,
			Reason:  "Phục bút đã đến lúc nhắc lại",
			Summary: fmt.Sprintf("Phục bút %q được gieo ở chương %d, đã cách %d chương; cần chú ý: %s", entry.ID, entry.PlantedAt, state.chapter-entry.PlantedAt, truncateRunes(entry.Description, 30)),
		})
		if len(items) >= maxThreads {
			break
		}
...
// agingForeshadow lọc các phục bút đã cũ hơn foreshadowAgingChapters, bỏ qua mục đã chọn và sắp xếp theo thời điểm gieo.
	...
	appendReview := func(review *domain.ReviewEntry) bool {
		...
		for i, miss := range review.ContractMisses {
			add(domain.RecallItem{
				...
				Reason:  "Bài học từ hợp đồng sáng tác",
				Summary: fmt.Sprintf("Chương %d có điểm thiếu trong hợp đồng: %s", review.Chapter, miss),
			})
			if len(items) >= 3 {
				return true
			}
		...
		for i, issue := range review.Issues {
			...
			case "", "warning", "error", "critical":
				add(domain.RecallItem{
					...
					Chapter: review.Chapter,
					Reason:  "Vấn đề từ đánh giá trước",
					Summary: fmt.Sprintf("Chương %d: %s", review.Chapter, truncateRunes(issue.Description, 36)),
				})
			}
			if len(items) >= 3 {
				return true
...
// foreshadowAgingChapters là số chương tối thiểu trước khi một phục bút được xem là đã cũ.
// Phục bút cũ được đưa vào story_threads để writer cân nhắc, không tự động thay đổi nội dung.
// Ngưỡng này giúp giảm tải ngữ cảnh mà vẫn giữ các tuyến cần nhớ.
...
// truncateRunes cắt chuỗi theo số rune.
