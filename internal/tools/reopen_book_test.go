...
// completedBook tạo một tiểu thuyết N chương đã hoàn tất (phase=complete, CompletedChapters=1..n).
...
func TestReopenBookReopensCompletedBook(t *testing.T) {
	...
	args, _ := json.Marshal(map[string]any{"chapters": []int{3, 1}, "reason": "dọn ký tự đặc biệt"})
	...
	if len(p.PendingRewrites) != 2 || p.PendingRewrites[0] != 3 || p.PendingRewrites[1] != 1 {
		t.Errorf("PendingRewrites = %v, want [3 1] (đưa vào hàng đợi nguyên thứ tự)", p.PendingRewrites)
	}
...
func TestReopenBookRejectsNonCompleteBook(t *testing.T) {
	// Sách đang viết (chưa hoàn tất) không được reopen
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	}
...
func TestReopenBookRejectsUnwrittenChapters(t *testing.T) {
	...
	// Chương 5 không tồn tại -> từ chối (thuộc viết tiếp/vượt phạm vi, nên đi theo luồng điều chỉnh dung lượng)
