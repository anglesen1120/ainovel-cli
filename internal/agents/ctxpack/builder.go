...
func buildWriterStoreSummaryText(s *store.Store, budgetTokens int) (string, bool, error) {
	...
	return "Nội dung sau đến từ store lưu trữ bền vững của tiểu thuyết, dùng để khôi phục ngữ cảnh viết sau khi nén.\n\n" + strings.Join(parts, "\n\n"), true, nil
}
...
func writerStoreSummarySections(state *writerStoreSummaryState) []writerStoreSection {
	return []writerStoreSection{
		...
		{heading: "Tóm tắt mạch truyện hiện tại", data: state.currentArcSummary},
		{heading: "Tóm tắt quyển hiện tại", data: state.currentVolSummary},
		{heading: "Ảnh chụp nhân vật", data: state.snapshots},
		{heading: "Chi tiết gợi mở đang hoạt động", data: state.foreshadow},
		{heading: "Vấn đề bản thảo cần sửa", data: state.pendingReviews},
