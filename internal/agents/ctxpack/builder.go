...
func (s *writerStoreSummaryState) warn(scope string, err error) {
	if s != nil && err != nil {
		s.warnings = append(s.warnings, fmt.Sprintf("%s đọc thất bại: %v", scope, err))
	}
}
...
func buildWriterStoreSummaryText(s *store.Store, budgetTokens int) (string, bool, error) {
	...
	return "Nội dung sau đến từ store bền vững của tiểu thuyết, dùng để khôi phục ngữ cảnh viết sau khi nén。\n\n" + strings.Join(parts, "\n\n"), true, nil
}
...
func writerStoreSummarySections(state *writerStoreSummaryState) []writerStoreSection {
	return []writerStoreSection{
		...
		{heading: "Dàn ý chương hiện tại", data: state.currentOutline},
		{heading: "Tóm tắt arc hiện tại", data: state.currentArcSummary},
		{heading: "Tóm tắt quyển hiện tại", data: state.currentVolSummary},
		{heading: "Ảnh chụp nhân vật", data: state.snapshots},
		{heading: "Foreshadowing đang hoạt động", data: state.foreshadow},
		{heading: "Vấn đề rà soát chờ sửa", data: state.pendingReviews},
		{heading: "Timeline gần đây", data: state.timeline},
