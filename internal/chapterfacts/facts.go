...
// Properties trả về các trường JSON Schema dùng chung cho facts đầy đủ của chương.
func Properties(includeFeedback bool) []schema.Prop {
	...
	foreshadow := schema.Object(
		schema.Property("id", schema.String("ID foreshadow")).Required(),
	...
	props := []schema.Prop{
		...
		schema.Property("timeline_events", schema.Array("Sự kiện timeline", timeline)).Required(),
		schema.Property("foreshadow_updates", schema.Array("Thao tác foreshadow", foreshadow)).Required(),
		...
		schema.Property("cast_intros", schema.Array("Vai phụ mới", schema.Object(
			schema.Property("name", schema.String("Họ tên")).Required(),
			schema.Property("brief_role", schema.String("Vai trò ngắn gọn")).Required(),
		))).Required(),
		schema.Property("hook_type", llmcontract.Nullable(schema.Enum("Hook cuối chương", domain.HookTypes()...))).Required(),
	...
	if includeFeedback {
		feedback := schema.Object(
			...
			schema.Property("suggestion", schema.String("Gợi ý điều chỉnh cho dàn ý tiếp theo")).Required(),
		)
		feedback["description"] = "Đối tượng gợi ý cho dàn ý tiếp theo; phải truyền trực tiếp JSON object, đừng truyền JSON đã stringify"
		props = append(props, schema.Property("feedback", llmcontract.Nullable(feedback)).Required())
	}
	return props
}

// Validate kiểm tra các ràng buộc xác định dùng chung cho submit thường và chỉnh sửa thủ công.
