...
// DraftStore quản lý dàn ý chương, bản nháp và bản cuối.
type DraftStore struct{ io *IO }

func NewDraftStore(io *IO) *DraftStore { return &DraftStore{io: io} }

// SaveChapterPlan lưu dàn ý chương vào drafts/{ch}.plan.json.
...
// LoadChapterPlan đọc dàn ý chương.
...
// SaveDraft lưu bản nháp toàn chương vào drafts/{ch}.draft.md.
...
// LoadDraft đọc bản nháp toàn chương.
...
// LoadChapterContent tải phần nội dung bản nháp chương và số chữ.
...
// SaveFinalChapter lưu nội dung cuối cùng của chương vào chapters/{ch}.md.
...
// LoadChapterText đọc nguyên văn bản cuối đã nộp.
...
// LoadChapterRange đọc các đoạn văn bản cuối trong phạm vi chỉ định.
...
// ExtractDialogue trích xuất đoạn thoại của nhân vật chỉ định từ các chương đã nộp.
// maxCompletedChapter được bên gọi truyền vào, để tránh phụ thuộc chéo miền.
...
// ExtractStyleAnchors trích xuất các đoạn tiêu biểu làm neo phong cách.
// maxCompletedChapter được bên gọi truyền vào, để tránh phụ thuộc chéo miền.
