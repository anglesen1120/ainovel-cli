...
// SignalStore quản lý các tệp tín hiệu dùng một lần (kết quả commit/review, trạng thái chờ khôi phục).
...
// LoadAndClearLastCommit đọc và xóa tín hiệu commit một cách nguyên tử, ngăn chặn cạnh tranh TOCTOU.
...
// ClearLastCommit xóa tệp tín hiệu commit.
func (s *SignalStore) ClearLastCommit() error {
	return s.io.RemoveFile("meta/last_commit.json")
}

// SavePendingCommit lưu trạng thái commit chương đang chờ khôi phục.
...
// LoadPendingCommit đọc trạng thái commit chương đang chờ khôi phục.
...
// ClearPendingCommit xóa trạng thái commit chương đang chờ khôi phục.
...
// LoadLastReviewSignal đọc tệp tín hiệu review.
...
// ClearLastReview xóa tệp tín hiệu review.
func (s *SignalStore) ClearLastReview() error {
	return s.io.RemoveFile("meta/last_review.json")
}

// LoadAndClearLastReview đọc và xóa tín hiệu review một cách nguyên tử.
...
// ClearStaleSignals dọn dẹp các tệp tín hiệu còn sót lại (gọi khi tiến trình khởi động lại).
