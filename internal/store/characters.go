...
// CharacterStore quản lý hồ sơ nhân vật và ảnh chụp nhanh trạng thái.
type CharacterStore struct {
	io      *IO
	outline *OutlineStore // phụ thuộc chỉ đọc, dùng để duyệt ảnh chụp nhanh
}
...
// SaveSnapshots lưu ảnh chụp nhanh trạng thái nhân vật vào meta/snapshots/v{vol}a{arc}.json.
...
// LoadSnapshots đọc ảnh chụp nhanh nhân vật của quyển và hồi được chỉ định.
...
// LoadLatestSnapshots tải ảnh chụp nhanh nhân vật gần nhất (tìm theo thứ tự ngược của quyển và hồi).
func (s *CharacterStore) LoadLatestSnapshots() ([]domain.CharacterSnapshot, error) {
	// Thiếu đề cương → (nil,nil) đã được xử lý bên trong LoadLayeredOutline; hỏng phải được đẩy lên,
	// nếu không ảnh chụp nhanh nhân vật sẽ âm thầm biến mất cùng với hư hỏng đề cương phân tầng,
	// việc viết truyện mất tính liên tục của nhân vật mà không ai hay biết.
...
func renderCharacters(chars []domain.Character) string {
	...
	for _, c := range chars {
		...
		fmt.Fprintf(&b, "%s\n\n", c.Description)
		if c.Arc != "" {
			fmt.Fprintf(&b, "**Tuyến phát triển nhân vật**：%s\n\n", c.Arc)
		}
		if len(c.Traits) > 0 {
			fmt.Fprintf(&b, "**Đặc trưng**：%s\n\n", strings.Join(c.Traits, "、"))
		}
	}
	return b.String()
}
