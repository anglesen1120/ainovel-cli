...
// TestUsageStore_LoadMissing kiểm tra file không tồn tại thì trả về (nil, nil), để bên gọi chạy replay.
...
// TestUsageStore_RoundTrip ghi rồi đọc lại, kiểm tra dữ liệu tích lũy quay về nguyên vẹn.
...
// TestUsageStore_LoadSchemaMismatch kiểm tra khi schema tương lai nâng cấp thì file cũ bị bỏ qua (để host chạy replay dựng lại),
// không nhét nhầm các trường không tương thích vào tracker.
func TestUsageStore_LoadSchemaMismatch(t *testing.T) {
	dir := t.TempDir()
	us := NewUsageStore(newIO(dir))

	// viết tay một mẫu dữ liệu cũ schema=0
