#!/bin/sh
# Kịch bản cài đặt ainovel-cli bằng một lệnh
#
...
case "$(uname -s)" in
	Darwin) OS="Darwin" ;;
	Linux)  OS="Linux" ;;
	*) echo "Hệ điều hành không được hỗ trợ: $(uname -s); trên Windows, hãy tải thủ công tại https://github.com/$REPO/releases"; exit 1 ;;
esac
...
[ -n "$TAG" ] || { echo "release thiếu tag_name; từ chối cài đặt"; exit 1; }
...
[ "$ACTUAL" = "$EXPECTED" ] || {
	echo "Xác minh SHA256 của $ASSET thất bại; từ chối cài đặt"
	exit 1
}
echo "Đã xác minh SHA256"
...
tar -xOzf "$TMP/$ASSET" "$BIN" > "$TMP/extract/$BIN" || {
	echo "Không thể trích xuất $BIN từ gói cài đặt; từ chối cài đặt"
	exit 1
}
[ -s "$TMP/extract/$BIN" ] || { echo "$BIN trong gói cài đặt rỗng; từ chối cài đặt"; exit 1; }
...
echo "✓ Đã cài đặt: $DEST/$BIN"
[ -n "$TAG" ] && echo "Phiên bản: $TAG"
command -v "$BIN" >/dev/null 2>&1 || echo "Lưu ý: $DEST không có trong PATH; hãy thêm vào PATH"
